...
```{r functions}
plot_tree <- function(bart_fit, title, data, tau = FALSE){
  if(tau == TRUE){
    tree_data <- data.frame(data, tau = colMeans(bart_fit$tau))
    tree_fit <- rpart(tau ~ ., data = tree_data)
  }
  else{
...
plot_het_effects <- function(bart_fit, title, col_num = 2, data){
  ...
  bart_true_est_means <- data.frame(means = c(bart_means, mu), 
                                    groups = c(rep("estimate", length(bart_means)), rep("true", length(mu))), 
                                    x = rep(data, 2))
...
plot_treat_effects <- function(bart_fit, title, col_num = 2, data){
  bart_means <- map_dbl(1:ncol(bart_fit$tau), function(c){mean(bart_fit$tau[,c])})
  bart_lower <- map_dbl(1:ncol(bart_fit$tau), function(c){quantile(bart_fit$tau[,c], 0.025)})
  bart_upper <- map_dbl(1:ncol(bart_fit$tau), function(c){quantile(bart_fit$tau[,c], 0.875)})
  
  bart_true_est_means <- data.frame(means = c(bart_means, tau), 
                                    groups = c(rep("estimate", length(bart_means)), rep("true", length(tau))), 
                                    x = rep(data, 2))
...
plot_sub_effects <- function(bart_fit, limit, model, col_num = 2, data, tau = FALSE){
  ...
  diff_plot <- ggplot(data = NULL) +
                geom_histogram(aes(X_means_diff)) +
                 ggtitle(paste0("Histogram of posterior draws of difference \n in group-specific impacts \n for ", model)) +
  ...
  if(tau == TRUE){
   # Step 1: identify groups
    prediction_tau <- t(bart_fit$tau) %>% 
      as.data.frame()
    colnames(prediction_tau) <- 1:2000
  
    tau_prediction_X <- prediction_tau %>% 
    bind_cols(data.frame(X = data[,col_num]))
  
    tau_X_lower <- tau_prediction_X %>% 
      filter(X < limit)
    
    tau_X_upper <- tau_prediction_X %>% 
      filter(X >= limit)
    
    # Step 2: take draw means
    tau_X_lower_means <- colMeans(tau_X_lower %>% select(-X))
    tau_X_upper_means <- colMeans(tau_X_upper %>% select(-X))
    tau_X_means <- data.frame(means = c(tau_X_lower_means, tau_X_upper_means), group = c(rep("lower", length(tau_X_lower_means)), rep("upper", length(tau_X_upper_means)))) %>% 
      mutate_at(vars(group), as.factor)
    
    
    # Step 3: take difference in means for each draw
    tau_X_means_diff <- tau_X_upper_means - tau_X_lower_means
    
    # Step 4: plot histogram of differences
    tau_diff_plot <- ggplot(data = NULL) +
                  geom_histogram(aes(tau_X_means_diff)) +
                  ggtitle(paste0("Histogram of posterior draws of difference  \n in group-specific impacts \n for ", model, "\n (treatment effects)")) +
                  geom_vline(xintercept = quantile(tau_X_means_diff, 0.025), color = "red") +
                  geom_vline(xintercept = quantile(tau_X_means_diff, 0.975), color = "red") +
                  geom_vline(xintercept = mean(tau_X_means_diff), color = "blue") +
                  xlab("Difference in means")
    
    return(grid.arrange(diff_plot, tau_diff_plot, yhats_plot, ncol = 2))
...
plot_tree(bcf_fit_edit, "Tree from BCF model (treatment effects)", data = x)
```


Based on this tree, we choose to investigate X3.
...
```{r bcf}
plot_treat_effects(bcf_fit, "Comparison of estimate vs actual heterogeneous effects with BCF", col_num = 3, data = x)
```
...
plot_sub_effects(bcf_fit, -0.3994, "BCF model", col_num = 3, data = x, tau = TRUE)
```
...
Based on this tree, we choose to investigate pihat.
...
### Differences in subgroup effects
...
### Tree for treatment effects
```{r computing treatment effects, include = FALSE}
treated_df <- as.data.frame(cbind(x, rep(1, length(pihat)), pihat))
colnames(treated_df) <- colnames(as.data.frame(x_nonbcf))

non_treated_df <- as.data.frame(cbind(x, rep(0, length(pihat)), pihat))
colnames(non_treated_df) <- colnames(as.data.frame(x_nonbcf))

bm_treated_predict <- bart_machine_get_posterior(bm_fit, treated_df)
bm_nontreated_predict <- bart_machine_get_posterior(bm_fit, non_treated_df)

bm_treatment_diff <- bm_treated_predict$y_hat_posterior_samples - bm_nontreated_predict$y_hat_posterior_samples

bm_post$tau <- t(bm_treatment_diff)
plot_tree(bm_post, "Tree from BartMachine model", data = x_nonbcf, tau = TRUE)
```

### Heterogeneous treatment effects

```{r bm treatment effects}
plot_treat_effects(bm_post, "Comparison of estimate vs actual heterogeneous effects with bartMachine", col_num = 3, data = x_nonbcf)
```

### Differences in subgroup treatment effects

Based on the tree, we decide to look at the differences between X3 smaller or greater than -0.2554

```{r bm difference in group-specific impacts for treatment effects}
plot_sub_effects(bm_post, -0.2554, "bartMachine model", 3, data = x_nonbcf, tau = TRUE)
```
...
#dbart_treated <- predict(dbart_fit, cbind(x, rep(1, length(pihat)), pihat))
