...
```{r} 
ggplot(calcofi_reg %>% mutate(graph_color = case_when(Depthm <= 500 ~ "blue", 
                                                      Depthm > 500 & Depthm < 1500 ~ "green",
                                                      Depthm >= 1500 ~ "red")), aes(x = Depthm, y = O2ml_L)) +
  geom_point(aes(color = graph_color)) +
  ggtitle("Cut-off for values of Depth for next graphs")
```

We decide to create a cut-off at Depth of 1000m.
We now show the distribution of O2 vs Depth for each section
```{r}
ggplot(calcofi_reg %>% mutate(cut_off = factor(case_when(Depthm <= 500 ~ "depth<=500", 
                                                      Depthm > 500 & Depthm < 1500 ~ "500<Depth<1500",
                                                      Depthm >= 1500 ~ "Depth>=1500"))), aes(y = T_degC, x = Salnty)) +
  geom_point() +
  facet_grid(.~cut_off) +
  ggtitle("Temperature vs Salinity for Depth greater or equal to 1000m")
```
...
**Question: do we need to scale Salinity and Temperature??**
